...
import { Server, Landmark, ScanSearch, Clock, FileCheck, Users } from "lucide-react";
  ...
  return (
    <section className="py-24 relative bg-black border-t-2 border-white/20">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        >
            <StatItem icon={<Clock className="text-red-500" />} text="Real Deadlines" />
            <StatItem icon={<FileCheck className="text-red-500" />} text="Reviewed Work" />
            <StatItem icon={<Users className="text-red-500" />} text="Team Collaboration" />
        </motion.div>
      </div>
    </section>
  );
...
function WarRoomCard({ title, icon, tag, description, color, delay }: { title: string, icon: React.ReactNode, tag: string, description: string, color: string, delay: number }) {
    ...
    const getColors = (c: string) => {
        switch(c) {
            ...
            case "blue": return "text-blue-500 border-blue-500 bg-blue-500/10";
    ...
    return (
        ...
        >
            <div className="flex justify-between items-start mb-6">
                 <div className={cn("p-3 border-2", themeColors.replace("bg-", "bg-opacity-0 "))}>
                    {icon}
                 </div>
                 <span className={cn("px-2 py-1 text-[10px] font-black uppercase tracking-widest border border-current", themeColors)}>
                    {tag}
                 </span>
            </div>
            
            <h3 className="text-2xl font-black text-white uppercase mb-4 group-hover:text-red-500 transition-colors">{title}</h3>
...
function StatItem({ icon, text }: { icon: React.ReactNode, text: string }) {
    return (
        <div className="flex items-center gap-3">
            {icon}
            <span className="text-sm font-mono text-gray-300 uppercase tracking-wider">{text}</span>
        </div>
    )
}
