...
import { Rocket, Monitor, BookOpen, Crown, Star, ShieldCheck, FileText } from "lucide-react";
    ...
    <section className="py-24 relative bg-black border-t-2 border-white/20">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="text-center mb-20">
          ...
          >
            The <span className="text-red-500">3-Tier</span> <span className="text-purple-500">Agency</span> Operations
          </motion.h2>
          ...
          >
            ...
            Placement is determined by Crucible performance.
          </motion.p>
        </div>
        ...
        <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="bg-gradient-to-r from-red-900/20 to-purple-900/20 border-2 border-red-500/30 p-8 md:p-10 mb-20 relative overflow-hidden"
        >
             <div className="flex flex-col md:flex-row gap-8 items-start relative z-10">
                ...
                <div className="flex-1">
                    <h3 className="text-2xl font-black text-white uppercase mb-2">The Top 5%</h3>
                    <p className="text-gray-400 mb-6 font-mono text-sm">Top 5% from all tiers get unique guidelines and personalized roadmap</p>
                    
                    <div className="flex flex-wrap gap-3">
                        {["LinkedIn optimization", "Resume development", "Portfolio development", "Project guiding and report", "Next company internship help and guide", "Referrals"].map((tag, i) => (
                            <span key={i} className="px-3 py-1 bg-black border border-white/20 text-xs font-mono text-gray-300 uppercase hover:border-white transition-colors cursor-default">
                                {tag}
                            </span>
                        ))}
                    </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <StatBox number="100%" label="Exposure Guarantee" highlight="text-purple-500" />
            <StatBox number="Top 5%" label="Elite Program" highlight="text-purple-500" />
            <StatBox number="NDA" label="Professional Contracts" highlight="text-purple-500" />
        </div>
      </div>
    </section>
  );
...
function TierCard({ tier, title, percent, subtitle, icon, badge, description, benefit, color, delay }: TierCardProps) {
    const colorMap = {
        red: {
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-start mb-6">
                <div className={cn("p-3 border-2 rounded-lg bg-black text-white", theme.border)}>
                    {icon}
                </div>
            </div>

            <div className="mb-6">
                <div className="text-xs font-mono text-gray-500 mb-1 uppercase">{tier}</div>
                <h3 className="text-2xl font-black text-white uppercase mb-2">{title}</h3>
            ...
            <div className="bg-white/5 border border-white/10 p-3 mb-6 flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-gray-200 uppercase">{badge}</span>
            </div>
            ...
            <div className={cn("mt-auto pt-6 border-t border-white/10 text-xs font-medium", theme.text)}>
                 <span className="font-bold mr-1 text-white/50">{'>'} Benefit:</span> {benefit}
            </div>
        </motion.div>
    )
}

function StatBox({ number, label, highlight }: { number: string, label: string, highlight?: string }) {
    return (
        <motion.div 
        ...
        >
            <div className={cn("text-3xl font-black mb-1", highlight || "text-white")}>{number}</div>
